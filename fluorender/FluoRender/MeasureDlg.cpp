...
void RulerListCtrl::Append(wxString id, wxString name_info, double length, wxString unit,
      double angle, wxString points, bool time_dep, int time, wxString desc)
{
   long tmp = InsertItem(GetItemCount(), id, 0);
   SetItem(tmp, 1, name_info);
...
void RulerListCtrl::Update(VRenderView* vrv)
{
   ...
   for (int i=0; i<(int)ruler_list->size(); i++)
   {
      ...
      Append(ruler->GetName(), ruler->GetNameDisp(), ruler->GetLength(), unit,
		  ruler->GetAngle(), points, ruler->GetTimeDep(), ruler->GetTime(), ruler->GetDesc());
   }
/*
