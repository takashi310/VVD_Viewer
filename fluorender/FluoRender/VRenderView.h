...
class VRenderGLView: public wxGLCanvas
{
   ...
   public:
      ...
      void Set3DRotCapture(double start_angle,
            ...
            int len);
      //time sequence data
      void Set4DSeqCapture(wxString &cap_file, int begin_frame, int end_frame, bool rewind, int cur_frame = -1);
      //batch files
...
class VRenderView: public wxPanel
{
   ...
   {
      if (m_glview)
         m_glview->Set4DSeqCapture(cap_file, begin_frame, end_frame, rewind, cur_frame);
   }
   void Set3DBatCapture(wxString &cap_file, int begin_frame, int end_frame)
   {
      if (m_glview)
