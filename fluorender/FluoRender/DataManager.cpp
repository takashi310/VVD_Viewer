...
VolumeData::~VolumeData()
{
	//m_vrの開放を先にしないとloadedbrks内の要素クリアができない
	if (m_vr)
		delete m_vr;
	if (m_tex && !m_dup)
		delete m_tex;
							  ...
							  double spcx, double spcy, double spcz)
{
	...
	if (bits == 8)
	{
		uint8 *val8 = (uint8 *)malloc(sizeof(uint8)*nx*ny*nz);
		if (!val8)
		{
	...
	{
		uint16 *val16 = (uint16 *)malloc(sizeof(uint16)*nx*ny*nz);
		if (!val16)
		{
			wxMessageBox("Not enough memory. Please save project and restart.");
			return;
...
void VolumeData::AddEmptyMask(uint8 inival)
{
	if (!m_tex || !m_vr)
		return;
	...
	if (m_tex->add_empty_mask())
	{
		...
		uint8 *val8 = (uint8 *)malloc(sizeof(uint8)*m_res_x*m_res_y*m_res_z);
		if (!val8)
		{
			wxMessageBox("Not enough memory. Please save project and restart.");
			return;
		...
		memset(val8, inival, sizeof(uint8)*m_res_x*m_res_y*m_res_z);
...
void VolumeData::AddEmptyLabel(int mode)
{
	...
	if (m_tex->add_empty_label())
	{
		...
		unsigned int *val32 = (unsigned int *)malloc(sizeof(unsigned int)*m_res_x*m_res_y*m_res_z);
		if (!val32)
		{
			wxMessageBox("Not enough memory. Please save project and restart.");
			return;
...
//brxmlのときは最下層のspacingを設定する（他の階層は自動的に計算）
...
void RulerBalloon::DrawBalloon(int nx, int ny, Point orig)
{
	...
	lh = fontHeight(ftype);
	margin_bottom += lh/2;//テキストのY座標がbaselineに一致するため
	maxlw = 0.0;
...
void Ruler::Draw(bool persp, int nx, int ny, vector<AnnotationDB> annotationdb, double spcx, double spcy, double spcz)
{
	...
	beginRenderText(2, 2, true);//glOrthoで左上が(0,0)右下が(2,2)になる
